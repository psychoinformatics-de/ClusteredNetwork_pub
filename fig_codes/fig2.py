...
def simulate_spontaneous(params):
    ...
    #print pylab.unique(spiketimes[0])
    #print pylab.unique(spiketimes[1])
    #print pylab.unique(spiketimes[2])
...
def plot_ff_cv_vs_jep(params,jep_range=pylab.linspace(1,4,41),jipfactor = 0.,reps = 10,spike_js = [1.,2.,3.,4.],spike_simtime = 1000.,markersize = 0.5,spikealpha = 0.5,plot_units = [0,4000],spike_randseed = 0,plot = True,redo_spiketrains = False):
    #print(params)
    ffs = []
    cv2s = []
    counts = []
    for jep in jep_range:
        if jipfactor != 0. or jep<10:
            ...
            #results = organiser.check_and_execute(params, simulate_spontaneous, datafile,reps = reps)
