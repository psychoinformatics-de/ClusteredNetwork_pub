...
def check_and_execute(params,func,datafile,key_list=None,reps = None,
    ...
    try:
        ...
        if key_list is None:
            key_list = key # [k for k in sorted(params.keys()) if k not in ignore_keys]
        ...
        if reps is None:
            ...
            # else:
            all_keys = []
            possible_keys = [key] if reps is None else key# [key+'_'+str(r) for r in range(reps)]
            ...
            if n_jobs>1:
                ...
                all_results = func(deepcopy(params))
                if save:
                    pickle.dump(all_results,open(full_datafile,'wb'),protocol = 2)
            ...
            #all_keys = all_results.keys()#sorted([k for k in list(all_results.keys()) if key in k])
            #print('New results', all_results[all_results.keys()[0]])
            #print('New results', len(all_results[all_results.keys()[0]]))
            #print('NEWWWW', all_keys)
            
            results = all_results#[all_results[k] for k in all_keys[:reps]]
