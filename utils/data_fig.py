...
def draw_hex_array(center,size=0.3,colors = [[0.5]*3]*7,axes = None,radius = 0.1,add = True,show_numbers = False,draw_center = True,lw = 1.):
...
def get_stats(gns,min_trials  =10,min_count_rate = 15,min_count=200,
    ...
    for gn in gns:
        for direction in range(1,7):
            #print gn,direction
            rate,trate = analyses.get_rate(gn,condition = 1,direction = direction,tlim  =tlim,alignment = alignment)
            rates.append(rate[0])
            trial_counts.append(analyses.get_trial_count(gn,1,direction))
            count_rates.append(analyses.get_mean_direction_counts(gn,1,direction,tlim  =tlim,alignment = alignment))
            ff,tff = analyses.get_ff(gn,condition = 1,direction = direction,window = window,tlim  =tlim,alignment = alignment)
            ffs.append(ff)
            cv2,tcv2 = analyses.get_corrected_cv2(gn,condition = 1,direction = direction,minvals = minvals,tlim  =tlim,alignment = alignment)
            cv2s.append(cv2)

            lv,tlv = analyses.get_lv(gn,condition = 1,direction = direction,tlim  =tlim,alignment = alignment)
            lvs.append(lv)
            cv_two,tcv_two = analyses.get_cv_two(gn,condition = 1,direction = direction,tlim  =tlim,alignment = alignment)
...
def plot_experiment(size,radius,direction =1,lw = 1.,y_pos = 0,condition = 1):

    colors = [off_gray]*6 + [yellow]
    draw_hex_array([0,y_pos],colors = colors,size = size,radius = radius,lw = lw)
    colors = [off_gray]*7
    if condition ==1:
    ...
    draw_hex_array([500,y_pos],colors = colors,size = size,radius = radius,lw = lw)

    colors = [off_gray]*7
    colors[direction] = red
    draw_hex_array([1500,y_pos],colors = colors,size = size,radius = radius,lw = lw)
