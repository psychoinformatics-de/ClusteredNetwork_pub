...
def _calc_cv2(params):
    ...
    result =  spiketools.time_warped_cv2(spiketimes,ot=params['ot'],rate = rate,tlim = params['tlim'],
                        kernel_width = params['kernel_width'],nstd=params['nstd'],pool = params['pool'],
                        bessel_correction = params['bessel_correction'],interpolate = params['interpolate'],
                        minvals = params.get('minvals',0),return_all = params.get('per_trial_correction',False))
    print('cv2: ',clock()-t0)
    return result
...
def get_population_decoding(gns,condition,window =400.,folds = 5,tlim = [0,2000],alignment = 'TS',
                            redo = False,reps = 10, classifier = 'LogisticRegression',classifier_args = {},n_jobs = 1):
    params = {'gns':tuple(sorted(gns)),'condition':condition,'alignment':alignment,
              'tlim':tlim,'window':window,'classifier':classifier,
              'folds':folds,'classifier_args':classifier_args}
    

    return organiser.check_and_execute(params, _calc_population_decoding, 'population_decoding_file',
